        ...
        for j, v in enumerate(nodes):
            if i != j:
                ...
                if distance is not None and distance <= R and distance > 0.0001:
                    TG[u] += (avg_centrality[u]['average_Closeness'] * avg_centrality[v]['average_Closeness']) / (distance ** 2)
...
with open('avg_closeness.txt') as f:
    data = f.read()
...
for v in verts:
    centrality_matrix[v] = {
        'average_Closeness': avg_statistics.get(v, 0)
    }

distance_type = 'tsd'

# avg Closeness Centrality is used as a Mass
...
temporal_gravity_dict_Closeness_tsd = {str(node): float(tg_value) for node, tg_value in TG.items()}
print("Temporal Gravity on Closeness_tsd:", temporal_gravity_dict_Closeness_tsd)
end_runtime = time.time()  # End timer
print(f"Runtime: {end_runtime - start_runtime:.4f} seconds")  # Print runtime


with open('TG_average_Closeness_tsd.txt', 'w') as file:
    file.write(json.dumps(temporal_gravity_dict_Closeness_tsd))
