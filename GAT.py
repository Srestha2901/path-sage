...
class GAT3(torch.nn.Module):
    def __init__(self, in_channels, hidden1=64, hidden2=32, out_channels=1, heads=1):
        super().__init__()
        self.gat1 = GATConv(in_channels, hidden1, heads=heads)
        self.gat2 = GATConv(hidden1 * heads, hidden2, heads=heads)
        self.gat3 = GATConv(hidden2 * heads, out_channels, heads=1)  # final layer single head
