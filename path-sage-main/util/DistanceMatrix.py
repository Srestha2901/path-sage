...
def temporal_shortest_distance(graph, source, target):
    try:
        if source == target:
            temporal_shortest_distance = 0
            return source, temporal_shortest_distance
        shortest_paths = list(nx.all_simple_paths(graph, source=source, target=target))
        #print("paths: ", shortest_paths)
        ...
        for path in shortest_paths:
            ...
            for i in range(len(path) - 2):
                ...
                v = path[i + 1]
                w = path[i+2]
                if graph[u][v][0]['time'] >= graph[v][w][0]['time']:
                    valid = False
                    break
            if valid:
                valid_shortest_paths.append(path)
                #print("valid_shortest_paths: ",valid_shortest_paths)
                shortest_path_lengths.append(len(path) - 1)

        if not shortest_path_lengths:
            return None, float('inf')

        temporal_shortest_distance = min(shortest_path_lengths)
        temporal_shortest_paths = [path for path in valid_shortest_paths if len(path) - 1 == temporal_shortest_distance]
        return temporal_shortest_paths, temporal_shortest_distance

    except nx.NetworkXNoPath:
        return None, float('inf')

def compute_distance_matrices(graph):
    nodes = sorted(graph.nodes, key=int)  # <- sort numerically
    n = len(nodes)
    ...
    for i, source in enumerate(nodes):
        for j, target in enumerate(nodes):
            #if source != target:
            t_paths, t_distance = temporal_shortest_distance(graph, source, target)
    ...
    return nodes, temporal_shortest_matrix, temporal_shortest_paths
input_file_path = 'input1.txt'
G1, G2, max_time, min_time, edges = ReadGraph.read_graph_from_file(input_file_path)

nodes, temporal_shortest_matrix, temporal_shortest_paths = compute_distance_matrices(G1)
...
with open('temporal_shortest_matrix.txt', 'w') as f:
    json.dump(temporal_shortest_matrix_dict, f, indent=4)

# Print nicely
print("Temporal Shortest Matrix (∞ = no path):")
print(temporal_shortest_matrix)

print("\nTemporal Shortest Paths:")
for key, val in temporal_shortest_paths.items():
    print(f"{key}: {val}")
end_time = time.time()
