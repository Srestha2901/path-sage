...
for v in verts:
    centrality_matrix[v] = {
        'agg_betweenness': agg_statistics.get(v, 0),
...
# agg betweenness Centrality is used as a Mass
...
temporal_gravity_dict_betweenness_agg_tsd = {str(node): float(tg_value) for node, tg_value in TG.items()}
print("Temporal Gravity on betweenness_agg_tsd:", temporal_gravity_dict_betweenness_agg_tsd)
end_runtime = time.time()  # End timer
print(f"Runtime: {end_runtime - start_runtime:.4f} seconds")  # Print runtime

with open('TG_Aggregated_betweenness_tsd.txt', 'w') as file:
    file.write(json.dumps(temporal_gravity_dict_betweenness_agg_tsd))
