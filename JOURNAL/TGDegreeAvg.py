...
def temporal_gravity_model3(G, avg_centrality, distance_type, R, fastest_arrival_matrix, temporal_shortest_matrix):
    ...
    avg_degrees = np.array([avg_centrality[node]['average_degree'] for node in nodes])
        ...
        for j in range(node_count):
            if i != j:
                ...
                if distance is not None and distance <= R and distance > 0.0001:
                    TG[nodes[i]] += (avg_degrees[i] * avg_degrees[j]) / (distance ** 2)
...
for v in verts:
    centrality_matrix[v] = {
        'average_degree': avg_statistics.get(v, 0)
    }

distance_type = 'tsd'

# avg degree Centrality is used as a Mass
...
temporal_gravity_dict_degree_tsd = {str(node): float(tg_value) for node, tg_value in TG.items()}
print("Temporal Gravity on degree_tsd:", temporal_gravity_dict_degree_tsd)
end_runtime = time.time()  # End timer
print(f"Runtime: {end_runtime - start_runtime:.4f} seconds")  # Print runtime

with open('TG_average_degree_tsd.txt', 'w') as file:
    file.write(json.dumps(temporal_gravity_dict_degree_tsd))