        ...
        for j, v in enumerate(nodes):
            if i != j:
                ...
                if distance is not None and distance <= R and distance > 0.0001:
                    TG[u] += (agg_centrality[u]['agg_PageRank'] * agg_centrality[v]['agg_PageRank']) / (distance ** 2)
...
with open('agg_PageRank.txt') as f:
    data = f.read()
...
for v in verts:
    centrality_matrix[v] = {
        'agg_PageRank': agg_statistics.get(v, 0),
    }
...
temporal_gravity_dict_PageRank_agg_tsd = {str(node): float(tg_value) for node, tg_value in TG.items()}
print("Temporal Gravity on PageRank_agg_tsd:", temporal_gravity_dict_PageRank_agg_tsd)
end_runtime = time.time()  # End timer
print(f"Runtime: {end_runtime - start_runtime:.4f} seconds")  # Print runtime

with open('TG_Aggregated_PageRank_tsd.txt', 'w') as file:
    file.write(json.dumps(temporal_gravity_dict_PageRank_agg_tsd))
