        ...
        for j, v in enumerate(nodes):
            if i != j:
                ...
                if distance is not None and distance <= R and distance > 0.0001:
                    TG[u] += (agg_centrality[u]['agg_Closeness'] * agg_centrality[v]['agg_Closeness']) / (distance ** 2)
...
R = 3
with open('agg_closeness.txt') as f:
    data = f.read()
...
for v in verts:
    centrality_matrix[v] = {
        'agg_Closeness': agg_statistics.get(v, 0),

    }

distance_type = 'tsd'

# agg Closeness Centrality is used as a Mass
...
temporal_gravity_dict_Closeness_agg_tsd = {str(node): float(tg_value) for node, tg_value in TG.items()}
print("Temporal Gravity on Closeness_agg_tsd:", temporal_gravity_dict_Closeness_agg_tsd)

end_runtime = time.time()  # End timer
print(f"Runtime: {end_runtime - start_runtime:.4f} seconds")  # Print runtime


with open('TG_Aggregated_Closeness_tsd.txt', 'w') as file:
    file.write(json.dumps(temporal_gravity_dict_Closeness_agg_tsd))
